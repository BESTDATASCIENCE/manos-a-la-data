...
dfexp=comercio_hispanoamerica_mundo%>%
  filter(nombre_pais_origen=='Perú',anio==2017,valor_exportado_dolares>0)   %>%     
...
paisexp2017=(comercio_hispanoamerica_mundo%>%
               filter(nombre_pais_origen=='Perú',anio==2017)%>%
...
pexp=
  ...
  filter(nombre_pais_origen=='Perú',
         anio==2017,
...
pais=pexp%>%filter( nombre_corto=="Minerales")%>%
  top_n(1,valor)%>%
...
wrap_plots(g_exp + p_exp,
           heights  =  c(3, 0.8))+
  plot_annotation(title = "Exportaciones de Perú 2017",
                  subtitle = "Los siguientes datos corresponden a las exportaciones realizada en el año 2017 por la República de Perú, adicionalmente se graficaron los seis paises 
